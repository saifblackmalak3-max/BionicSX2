        ...
        Form {
            Section("Renderer") {
                Picker("Renderer", selection: $settings.renderer) {
                    ...
                    Text("Vulkan (MoltenVK)").tag(14)
                    Text("Software").tag(13)
                    Text("Null (No Output)").tag(11)
                }
                Text("Metal is recommended. Vulkan is an alternative hardware renderer. Software is slow but accurate. Null disables rendering (for testing). Requires restart.")
                    .font(.caption)
            ...
            Section("Filtering") {
                ...
                Text("Fast anti-aliasing. Smooths edges but may blur textures slightly.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Toggle("Load Custom Textures", isOn: $settings.loadTextureReplacements)
                Text("Replace game textures with custom ones from textures/ folder. Requires restart.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
