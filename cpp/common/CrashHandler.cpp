...
static const char* GetSignalNameUnsafe(int signal)
{
	switch (signal)
	{
	case SIGSEGV:
...
static bool BuildCrashReportPath(char* out_path, size_t size)
{
	const char* logs = EmuFolders::Logs.c_str();
	if (!logs || logs[0] == '\0')
	{
		if (size <= 16)
			return false;
		std::memcpy(out_path, "crash_report.txt", 16);
		out_path[16] = '\0';
		return true;
	}

	size_t logs_len = std::strlen(logs);
	const char* filename = "crash_report.txt";
	size_t filename_len = 16;
	const bool has_sep = logs[logs_len - 1] == '/' || logs[logs_len - 1] == '\\';
	size_t required = logs_len + filename_len + (has_sep ? 0 : 1) + 1;
	if (required > size)
		return false;

	std::memcpy(out_path, logs, logs_len);
	if (!has_sep)
	{
		out_path[logs_len++] = '/';
	}
	std::memcpy(out_path + logs_len, filename, filename_len);
	out_path[logs_len + filename_len] = '\0';
	return true;
}
...
void CrashHandler::CrashSignalHandler(int signal, siginfo_t* siginfo, void* ctx)
{
	...
	if (!BuildCrashReportPath(report_path, sizeof(report_path)))
	{
		std::memcpy(report_path, "crash_report.txt", 16);
		report_path[16] = '\0';
	}

	const time_t now = std::time(nullptr);
	const char* signal_name = GetSignalNameUnsafe(signal);
	const void* fault_addr = siginfo ? siginfo->si_addr : nullptr;
	char report_buf[512];
	const int report_len = std::snprintf(report_buf, sizeof(report_buf),
		"Signal: %s\n"
		"Fault address: %p\n"
		"Program counter: %p\n"
		"Timestamp: %lld\n",
		signal_name, fault_addr, exception_pc, static_cast<long long>(now));

	if (report_len > 0)
	{
		int fd = open(report_path, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644);
		if (fd != -1)
		{
			ssize_t bytes = write(fd, report_buf, static_cast<size_t>(report_len));
			(void)bytes;
			close(fd);
		}
	}
