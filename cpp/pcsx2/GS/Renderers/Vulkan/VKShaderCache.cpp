...
std::optional<VKShaderCache::SPIRVCodeVector> VKShaderCache::CompileShaderToSPV(u32 stage, std::string_view source, bool debug)
{
    ...
    if (!shader->parse(&glslang::DefaultTBuiltInResource, default_version, profile, false, true, messages, includer))
    {
        ...
        Console.Error("Vulkan shader compilation failed - consider switching to Metal renderer for better iOS compatibility");
        return std::nullopt;
    }
    ...
    if (!program->link(messages))
    {
        ...
        Console.Error("Vulkan shader linking failed - consider switching to Metal renderer for better iOS compatibility");
        return std::nullopt;
    }
    ...
    if (!intermediate)
    {
        ...
        Console.Error("Vulkan SPIR-V generation failed - consider switching to Metal renderer for better iOS compatibility");
        return std::nullopt;
    }
